export default class Piklor {
  elm!: HTMLElement | Element | null;
  cbs!: Function[];
  color!: string;
  isOpen!: boolean;
  colors!: string[];
  options!: {
    open: HTMLElement | Element | null;
    openEvent: string;
    style: {
      display: string;
    };
  ...
  // pick!: Event;
  pfx: string = "piklor-"; // Prefix for CSS classes, if needed
  /**
   * Piklor
  ...
  constructor(
    ...
    if (options.open) {
      options.open.addEventListener(options.openEvent, () => {
    ...
    if (self.elm)
      self.elm.addEventListener("click", (ev) => {
        var col = (ev?.target as HTMLElement)?.getAttribute("data-col");
        if (!col) {
          return;
        }
        self.color = col;
   ...
   */
  getElm(el: string | Element): HTMLElement | Element | null {
    if (typeof el === "string") {
  ...
  render() {
    ...
    if (self.elm) self.elm.innerHTML = html;
  }
   ...
   */
  close() {
    if (this.elm) (this.elm as HTMLElement).style.display = "none";
    this.isOpen = false;
  }
  ...
  open() {
    if (this.elm)
      (this.elm as HTMLElement).style.display = this.options.style.display;
    this.isOpen = true;
  }
